...
void run_workload(char *workload, int num_of_extra_links, unsigned long *time_arr, unsigned long *space_arr, int code){
	...
	LL_init(num_of_extra_links, num_ops, 2 * num_ops, num_ops);
	...
	for(i = 1; i <= num_ops; ++i){
		...
		(op == 1) ? LL_insert(di, VS_invalid()) : LL_delete(di, VS_invalid());
	...
	if(code == 2){
		...
		gettimeofday(&ts_end, NULL);
		elapsed_time = (ts_end.tv_sec * 1000000 + ts_end.tv_usec) - (ts_start.tv_sec * 1000000 + ts_start.tv_usec);
		strcpy(access_output, "../../output/linked-list-access.csv");
...
void populate_output_filename(char *output, char *workload, int num_of_extra_links){
	...
	num = atoi(num_str);
	sprintf(output, "../../output/cspot-linked-list-per-operation-%d-%d.csv", num, num_of_extra_links);
