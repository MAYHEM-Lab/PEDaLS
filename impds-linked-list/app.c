...
void run_workload(char *workload, int num_of_extra_links, unsigned long *time_arr, unsigned long *space_arr, int code){
	...
	ll = IMLL_init(num_of_extra_links, 1);

	elapsed_time = 0;
	for(i = 1; i <= num_ops; ++i){
		fscanf(fp, "%d %d", &op, &val);
		gettimeofday(&ts_start, NULL);
		(op == 1) ? IMLL_insert(ll, val) : IMLL_delete(ll, val);
	...
	if(code == 2){
		gettimeofday(&ts_start, NULL);
		IMLL_traverse(ll, &num);
		gettimeofday(&ts_end, NULL);
		elapsed_time = (ts_end.tv_sec * 1000000 + ts_end.tv_usec) - (ts_start.tv_sec * 1000000 + ts_start.tv_usec);
		strcpy(access_output, "../output/impds-linked-list-access.csv");
...
void populate_output_filename(char *output, char *workload, int num_of_extra_links){
	...
	num = atoi(num_str);
	sprintf(output, "../output/impds-linked-list-per-operation-%d-%d.csv", num, num_of_extra_links);
...
int main(int argc, char *argv[]){
	...
	MBuffSize = 100 * num_ops * sizeof(IMLLNODE) * (1 + num_of_extra_links) * sizeof(IMLLLINK);
